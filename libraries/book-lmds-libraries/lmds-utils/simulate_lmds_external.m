function simulate_lmds_external(target, limits, influence, phi_c, A, upper, Cof)
...
hs = plot_ds_model_ext(fig1, ds_lin, limits,'high'); hold on;
% axis tight
...
% Function controlling the state-based influence of modulation
exp_funct = @(x) exp_loc_act(influence, c, x);
plot_exp_2d(fig1, exp_funct); hold on;

% Function controlling the external-based influence of modulation
exter_funct = @(x) soft_step_fun(upper, x, target, Cof);
plot_poly(fig1, exter_funct,limits); hold on


% Define our reshaped dynamics (with externally modulated function)
reshaped_ds = @(x) lmds_2d_external(ds_lin, exp_funct, exter_funct, x, phi_c);
...
hs = plot_ds_model_ext(fig1, reshaped_ds, limits, 'high');
