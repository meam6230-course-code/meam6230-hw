...
classdef Shapes < matlab.mixin.Heterogeneous & handle
   ...
   methods
       ...
       function self = Shapes(gamma, gammaDistance, gradientGamma, parameters, position, velocityFunction, rho, noisy_sigma)
           ...
           if nargin > 0
              ...
              self.noisy_position = position;

              self.shapeParameters = parameters;
              self.prevTime = 0;

              self.gamma = @(x, y, z) gamma(x-self.noisy_position(1), ...
                                            y-self.noisy_position(2), ...
                                            z-self.noisy_position(3), ...
                                            parameters);
              self.gammaDistance = @(x, y, z) gammaDistance(x-self.noisy_position(1), ...
                                                            y-self.noisy_position(2), ...
                                                            z-self.noisy_position(3), ...
                                                            parameters);

              self.gradientGamma = @(x, y, z) gradientGamma(x-self.noisy_position(1), ...
                                                            y-self.noisy_position(2), ...
                                                            z-self.noisy_position(3), ...
                                                            parameters);
        ...
        function moveShape(self, t)
            ...
            self.noisy_position(1) = new_position(1) + self.noisy_sigma * randn(1,1);
            self.noisy_position(2) = new_position(2) + self.noisy_sigma * randn(1,1);
            self.noisy_position(3) = new_position(3) + self.noisy_sigma * randn(1,1);
            self.velocity(1) = new_velocity(1) + self.noisy_sigma * randn(1,1);
            self.velocity(2) = new_velocity(2) + self.noisy_sigma * randn(1,1);
            self.velocity(3) = new_velocity(3) + self.noisy_sigma * randn(1,1);
