...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clc; clear; close all;
initialPosition = [0.3; 0.4; 0.5];
myWorld = Environment(initialPosition, 0.0);
...
attractor = [0.3; -0.4; 0.5];
multi_closest = false;
if multi_closest
    dsModulated = @(x) modulatedDS(x, attractor, myWorld);
else

    dsModulated = @(x) modulatedDS_closest(x, attractor, myWorld);
end
...
function xdot = modulatedDS_closest(x, attractor, world, nominalDS)

    if nargin < 4
        xdot_nominal = 0.1*-4*eye(3)*(x-attractor);
    else
        xdot_nominal = nominalDS(x);
    end
    ...
    % Closest obstacle modulation strategy
